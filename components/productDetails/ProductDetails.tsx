...
import { AiOutlineHeart, AiOutlineMinus, AiOutlinePlus, AiOutlineRight } from "react-icons/ai"
...
export default function ProductDetails({ product }: { product: Product }) {
  const tabs = ["Description", "Customer Reviews", "Shipping & Returns", "Return Policies"]
              ...
              {product?.images?.map((ele, i) => (
                <SwiperSlide key={i}>
                  <div
                    ...
                    onMouseMove={() =>
                      setActiveMagnifyImg(ele)
                    }
                  >
                    {/* <div className="relative aspect-square"> */}
                    <Image src={ele} alt="Thumbnail" fill className="w-full h-full object-cover" />
                    {/* </div> */}
                  </div>
                </SwiperSlide>
              ))}
            ...
            <ReactImageMagnify
              {...{
                ...
                enlargedImageContainerDimensions: {
                  // width: '200%', // Zoomed image width
                  // height: '100%' // Zoomed image height
                  width: 600,
                  height: 600,
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-sm text-gray-500">5 sold in last 24 hours</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm text-white bg-red-500 rounded-full px-1.5 font-medium">-21%</span>
          </div>

          <div className="bg-gray-50 p-4 rounded-lg">
            <p className="text-sm text-gray-600">
              The garments labelled as Committed are products that have been produced using sustainable fibres or
            ...
            <div className="flex gap-2">
              {product?.colors.map((color) => (
                <button
                  ...
                  className={`h-6 w-6 rounded-full ${selectedColor === color.name ? "ring-2 ring-[#9333EA] ring-offset-2" : ""
                    }`}
              ...
              product?.sizes.length > 0 && (
                product?.sizes?.map((size) => (
                  <button
                    ...
                    className={`flex h-10 w-10 items-center justify-center rounded border ${selectedSize === size
          ...
          <button className="w-full bg-red-500 text-white py-4 rounded-lg hover:bg-red-600">BUY IT NOW</button>

          <div className="flex gap-6 py-4 border-t border-b">
            <button className="flex items-center gap-2 text-sm">
              <FiShare2 className="w-4 h-4" /> Share
            </button>
            <button className="flex items-center gap-2 text-sm">Ask A Question</button>
            <button className="flex items-center gap-2 text-sm">Delivery & Return</button>
          </div>
      ...
      <div>
        ...
        >
          {data.length > 0 ? (
            ...
            (
              <div className="flex justify-center"><p> "No Products" </p></div>
            )
          }
        </Swiper>
      </div>
